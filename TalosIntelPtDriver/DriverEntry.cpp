...
const LPTSTR g_lpDevName = L"\\Device\\TalosIntelPT";
const LPTSTR g_lpDosDevName = L"\\DosDevices\\TalosIntelPT";
...
VOID UnloadPtDpc(struct _KDPC *Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2) 
{
	...
	DbgPrint("[TalosIntelPt] Stopping and unloading the Trace for CPU #%i...\r\n", dwCurProc);
...
VOID DriverUnload(PDRIVER_OBJECT pDrvObj) 
{
	...
	for (DWORD i = 0; i < g_pDrvData->dwNumProcs; i++) 
	{
		...
		if (!NT_SUCCESS(ntStatus)) 
		{
			...
			KeBugCheckEx(PROCESS_HAS_LOCKED_PAGES, 0x00, (ULONG_PTR)procData->lpMappedProc, procData->qwBuffSize / PAGE_SIZE, 0);
		}
	...
	DbgPrint("[TalosIntelPT] driver successfully unloaded.");
	RevertToDefaultDbgSettings();
}